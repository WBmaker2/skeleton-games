...
function placeholderFace(): string {
  return `<svg class="art-fallback" viewBox="0 0 200 200" aria-hidden="true" focusable="false">`
    + `<circle cx="100" cy="105" r="58" fill="#ffffff" stroke="#14081f" stroke-width="7"/>`
    + `<circle cx="80" cy="95" r="9" fill="#14081f"/>`
    + `<circle cx="120" cy="95" r="9" fill="#14081f"/>`
    + `<circle cx="83" cy="92" r="3" fill="#ffffff"/>`
    + `<circle cx="123" cy="92" r="3" fill="#ffffff"/>`
    + `<path d="M78 122 Q100 140 122 122" fill="none" stroke="#14081f" stroke-width="7" stroke-linecap="round"/>`
    + `<circle cx="62" cy="112" r="7" fill="#ff71ce"/>`
    + `<circle cx="138" cy="112" r="7" fill="#ff71ce"/>`
    + `</svg>`;
}
...
function card(game: LandingGame): string {
  return `<li>`
    ...
    + `<a class="game-card" style="--card-accent: ${game.accent}" href="#/${game.id}">`
    ...
    + `</div>`
    + `<span class="game-card__badge">GAME ${game.no} · ${game.effect}</span>`
...
export function renderLanding(app: HTMLElement): void {
  ...
  app.innerHTML =
    ...
    + `<p class="landing__sub">마음에 드는 카드를 골라 시작하세요. 카메라 앞에 서거나, 키보드와 마우스로도 놀 수 있어요.</p></header>`
