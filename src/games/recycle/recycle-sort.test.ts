...
describe('RecycleSort grab-and-carry', () => {
  ...
  it('spawns trash at the center bottom', () => {
    const g = new RecycleSort();
    g.start();
    expect(g.item.x).toBe(320);
    expect(g.item.y).toBe(300);
    expect(g.carriedBy).toBeNull();
  });
  it('grabs trash when a hand touches it', () => {
    const g = new RecycleSort();
    g.start();
    const events = g.tick(frame([...torso(320), ...hands(320, 300)]), 16);
  ...
  it('carries the item with the hand', () => {
    const g = new RecycleSort();
    g.start();
    g.tick(frame([...torso(320), ...hands(320, 300)]), 16);
  ...
  it('sorts plastic carried to the left bin', () => {
    ...
    const seen: string[] = [];
    for (let i = 0; i < 40; i++) {
      seen.push(...g.tick(frame([...torso(100), ...hands(100, 300)]), 16).map((e) => e.type));
    }
